-- [[ ========================================================= ]] --
-- [[ KZOYZ HUB - AUTO FARM & AUTO COLLECT (CLASSIC CORE)       ]] --
-- [[ ========================================================= ]] --

local TargetPage = ...
if not TargetPage then warn("Module harus di-load dari Kzoyz Index!") return end

getgenv().ScriptVersion = "Auto Farm v5.0 (Auto Collect)" 
...
getgenv().AutoCollect = false; -- 🌟 FITUR BARU!
getgenv().OffsetX = 0; 
getgenv().OffsetY = 0; 
...
function CreateToggle(Parent, Text, Var) 
    local Btn = Instance.new("TextButton"); Btn.Parent = Parent; Btn.BackgroundColor3 = Theme.Item; Btn.Size = UDim2.new(1, -10, 0, 35); Btn.Text = ""; Btn.AutoButtonColor = false; 
...
function CreateSlider(Parent, Text, Min, Max, Default, Var) 
    ...
    local SliderBg = Instance.new("TextButton"); SliderBg.Parent = Frame; SliderBg.BackgroundColor3 = Color3.fromRGB(30, 30, 30); SliderBg.Position = UDim2.new(0, 10, 0, 28); SliderBg.Size = UDim2.new(1, -20, 0, 6); SliderBg.Text = ""; SliderBg.AutoButtonColor = false; 
    local SC = Instance.new("UICorner"); SC.CornerRadius = UDim.new(1,0); SC.Parent = SliderBg; 
...
CreateToggle(TargetPage, "Auto Collect", "AutoCollect") -- 🌟 SAKLAR BARU
...
-- 1. LOOP AUTO PLACE (MURNI LOGIKA v1.1 MILIKMU)
task.spawn(function() 
    while true do 
...
-- 2. LOOP AUTO BREAK (MURNI LOGIKA v1.1 MILIKMU)
task.spawn(function() 
    while true do 
...
-- 3. LOOP AUTO COLLECT (FITUR BARU UNTUK NYEDOT DROP)
task.spawn(function()
    while true do
        if getgenv().AutoCollect then
            ...
            if hrp then
                -- Targetkan barang jatuh ke posisi pemain secara instan
                ...
                if dropFolder then
                    ...
                    -- Fallback: Jika tak ada folder khusus, sedot part liar di luar map
        ...
        task.wait(0.1) -- Sedot secepat kilat (setiap 0.1 detik)
    end
end)
