...
getgenv().ScriptVersion = "Auto Plant v3.0 - Tile Scanner" 
...
-- [[ FUNGSI MEMBACA WORKSPACE.TILES (PETA VIRTUAL) ]] --
local function GetWorldMap()
    local map = {}
    for x = 0, 99 do map[x] = {} end -- Bikin array grid X: 0-99
    ...
    if tilesFolder then
        for _, block in ipairs(tilesFolder:GetChildren()) do
            if block:IsA("BasePart") then
                ...
                if gX >= 0 and gX <= 99 and gY >= 0 and gY <= 59 then
                    map[gX][gY] = true -- Menandai kalau kotak ini ADA ISINYA (Tanah / Sapling / Benda)
                end
            end
        end
    end
...
-- [[ SISTEM JALAN PINTAR DENGAN PETA ]] --
local function SmartWalk(tX, tY, map)
    local H = workspace.Hitbox:FindFirstChild(LP.Name)
    if not H then return end
    local startZ = H.Position.Z

    while getgenv().EnableTilePlant do
        ...
        if cY < tY then -- JIKA HARUS NAIK
            if map[cX] and map[cX][cY+1] then -- Atapnya mentok!
                getgenv().ModFly = false
                
                -- Cari jalan ke ujung (Celah kosong)
                local gapX = cX
                local gapFound = false
                local searchDist = 1
                
                while searchDist <= 100 do
                    local rightX = cX + searchDist
                    local leftX = cX - searchDist
                    
                    if rightX <= 99 and map[rightX] and not map[rightX][cY+1] then
                        gapX = rightX; gapFound = true; break
                    elseif leftX >= 0 and map[leftX] and not map[leftX][cY+1] then
                        gapX = leftX; gapFound = true; break
                    end
                    searchDist = searchDist + 1
                end
                
                if not gapFound then 
                    warn("Atap ketutup full, ga bisa manjat!")
                    getgenv().EnableTilePlant = false
                    break 
                end
                ...
                -- Jalan ke arah celah
                if cX ~= gapX then
                    cX = cX + (gapX > cX and 1 or -1)
                end
            else
                -- Atap bolong, Mod Fly nyala, lompat naik!
                getgenv().ModFly = true
                cY = cY + 1
            end
        elseif cY > tY then -- JIKA HARUS TURUN
            getgenv().ModFly = false -- Matiin fly biar jatuh
            cY = cY - 1
        else -- JALAN HORIZONTAL BIASA
            getgenv().ModFly = false
            if cX ~= tX then
...
CreateToggle(TargetPage, "🤖 START AUTO PLANT (TILE SCAN)", "EnableTilePlant")

-- [[ LOGIC AUTO PLANT ]] --
local RemotePlace = RS:WaitForChild("Remotes"):WaitForChild("PlayerPlaceItem")

getgenv().KzoyzPlantLoop = task.spawn(function()
    -- Pola Kiri-Kanan yang dibalik setiap naik baris
    local moveLeftToRight = true 
    
    while true do
        if getgenv().EnableTilePlant then
            local seedSlot = GetSlotByItemID(getgenv().PlantSeedID)
            if not seedSlot then 
                getgenv().EnableTilePlant = false; getgenv().ModFly = false
            ...
            -- Cari baris terendah (Mulai dari posisi player ke atas) yang masih ada lahan kosong
            local targetY = nil
            for y = myY, 59 do
                local hasEmptyDirt = false
                for x = 0, 99 do
                    -- Syarat Tanam: Posisi [x, y] Kosong, TAPI bawahnya [x, y-1] Ada Block (Pijakan)
                    if WorldMap[x] and not WorldMap[x][y] and WorldMap[x][y-1] then
                        hasEmptyDirt = true; break
                    end
                end
                if hasEmptyDirt then targetY = y; break end
            end
            
            if targetY then
                -- Atur arah jalan (Zig Zag)
                local startX = moveLeftToRight and 0 or 99
                local endX = moveLeftToRight and 99 or 0
                local stepX = moveLeftToRight and 1 or -1
                
                for x = startX, endX, stepX do
                    if not getgenv().EnableTilePlant then break end
                    
                    -- Cek ulang grid ini pake data Map
                    if WorldMap[x] and not WorldMap[x][targetY] and WorldMap[x][targetY-1] then
                        SmartWalk(x, targetY, WorldMap)
                        task.wait(0.05)
                        RemotePlace:FireServer(Vector2.new(x, targetY), seedSlot)
                        task.wait(getgenv().PlaceDelay)
                        WorldMap[x][targetY] = true -- Tandain kalau udah ditanam biar gak double loop
                    end
                end
                
                -- Setelah 1 baris beres, balik arahnya buat baris atasnya
                moveLeftToRight = not moveLeftToRight
            else
                -- Kalau targetY kosong, berarti ladang di atas udah keisi semua.
                getgenv().EnableTilePlant = false
                getgenv().ModFly = false
            end
        end
...
end)        if cX == tX and cY == tY then break end -- Udah sampai
