...
getgenv().ScriptVersion = "Pabrik v0.70-SmartCollect" 
...
getgenv().AutoCollectPabrik = true 
...
CreateToggle(TargetPage, "👻 Smart Auto Collect (Pabrik)", "AutoCollectPabrik")
...
task.spawn(function()
    while true do
        if getgenv().EnablePabrik then
            ...
            -- FASE 3: HARVESTING (DENGAN SMART COLLECT SWEEP)
            if getgenv().EnablePabrik then
                WalkToGrid(getgenv().PabrikStartX, getgenv().PabrikYPos, true); task.wait(0.5)
                for x = getgenv().PabrikStartX, getgenv().PabrikEndX do
                    if not getgenv().EnablePabrik then break end
                ...
                if getgenv().EnablePabrik then
                    if getgenv().AutoCollectPabrik then
                        ...
                        local startX = math.min(getgenv().PabrikStartX, getgenv().PabrikEndX)
                        local endX = math.max(getgenv().PabrikStartX, getgenv().PabrikEndX)
                        
                        for x = startX, endX do
                            if CheckDropsAtGrid(x, getgenv().PabrikYPos) then 
                                table.insert(TilesToCollect, {x = x, y = getgenv().PabrikYPos}) 
                            end
                        end
                        
                        if #TilesToCollect > 0 and getgenv().EnablePabrik then
                            local char = LP.Character
                            ...
                            for _, tile in ipairs(TilesToCollect) do
                                if not getgenv().EnablePabrik then break end
                                WalkToGrid(tile.x, tile.y, true)
                                local waitTimeout = 0
                                while CheckDropsAtGrid(tile.x, tile.y) and waitTimeout < 15 and getgenv().EnablePabrik do
            ...
            -- FASE 4: AUTO FARM BLOCK (Seperti Master Auto Farm)
            if getgenv().EnablePabrik then
                ...
                while getgenv().EnablePabrik do
                    ...
                    -- 3. SMART COLLECT (GHOSTING)
                    if getgenv().AutoCollectPabrik and CheckDropsAtGrid(BreakTarget.X, BreakTarget.Y) then
                        local char = LP.Character
                        local hrp = char and char:FindFirstChild("HumanoidRootPart")
                        local hum = char and char:FindFirstChildOfClass("Humanoid")
                        local HitboxFolder = workspace:FindFirstChild("Hitbox")
